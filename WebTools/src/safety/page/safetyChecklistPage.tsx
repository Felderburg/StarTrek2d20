...
const SafetySectionView: React.FC<ISafetySectionViewProperties> = ({
  section,
  evaluation,
}) => {
      ...
      <table className="table table-dark">
        ...
        <thead>
          <tr>
            ...
            {SafetyEvaluation.allTypes.map((t) => (
              <th
                className="bg-black"
                key={'section-header-' + section.type + '-' + t.type}
              >
                {t.localizedName}
              </th>
            ))}
              ...
              <tr key={'section-category-' + category}>
                ...
                <td className="text-center">
                  <input
                    ...
                    value="AlwaysOk"
                    checked={
                      evaluation[category] === SafetyEvaluationType.AlwaysOk
                    }
                    onChange={() =>
                      select(category, SafetyEvaluationType.AlwaysOk)
                    }
                  />
                </td>
                <td className="text-center">
                  <input
                    ...
                    value="YellowAlert"
                    checked={
                      evaluation[category] === SafetyEvaluationType.YellowAlert
                    }
                    onChange={() =>
                      select(category, SafetyEvaluationType.YellowAlert)
                    }
                  />
                </td>
                <td className="text-center">
                  <input
                    ...
                    value="RedAlert"
                    checked={
                      evaluation[category] === SafetyEvaluationType.RedAlert
                    }
                    onChange={() =>
                      select(category, SafetyEvaluationType.RedAlert)
                    }
                  />
                </td>
              </tr>
...
const SafetyChecklistPage: React.FC<ISafetyChecklistPageProperties> = ({
  ...
  const exportPdf = async () => {
    ...
    import(
      /* webpackChunkName: 'export' */ '../../exportpdf/generatedSafetypChecklistSheet'
    ).then(({ GeneratedSafetyChecklistSheet }) => {
        ...
        <div className="page">
          <div className="container ms-0">
            ...
            <div className="row row-cols-1 row-cols-lg-2 my-3">
              {SafetySections.instance.sections.map((s) => (
                <SafetySectionView
                  section={s}
                  key={'section-' + s.type}
