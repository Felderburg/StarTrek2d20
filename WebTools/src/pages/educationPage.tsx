...
const EducationPageBase: React.FC<ICharacterProperties> = ({ character }) => {
  ...
  const selectTrack = (track: TrackModel) => {
    const enlisted =
      tab === StarfleetTrackTab.Enlisted ||
      track.id === Track.Enlisted ||
      track.enlisted;
  ...
  const toTableRow = (track: TrackModel, i: number) => {
    return (
      <tr
        key={i}
        onClick={() => {
          if (Window.isCompact()) selectTrack(track);
        }}
      >
        <td className="selection-header">
          {character.version === 1
            ? track.localizedName
            : track.localizedName2e}
        </td>
  ...
  const types =
    randomTrack != null && tab !== StarfleetTrackTab.Other
      ? toTableRow(
  ...
  return (
    <div className="page container ms-0">
      ...
      <main>
        <Header>
          {t(makeKey('EducationPage.type.', CharacterType[character.type]))}
        </Header>

        <InstructionText
          text={t(
            makeKey(
              'EducationPage.instruction.',
        ...
        character.version === 1 ? (
          <div
            className="btn-group w-100 my-4"
            role="group"
          ...
          >
            <button
              ...
              className={
                ...
                (tab === StarfleetTrackTab.Officer ? 'active' : '')
              }
              onClick={() => setTab(StarfleetTrackTab.Officer)}
            >
              {t('EducationPage.starfleet.academy')}
            </button>
            <button
              type="button"
              className={
                'btn btn-info btn-sm p-2 text-center ' +
                (tab === StarfleetTrackTab.Enlisted ? 'active' : '')
              }
              onClick={() => setTab(StarfleetTrackTab.Enlisted)}
            >
              {t('EducationPage.starfleet.enlisted')}
            </button>
            {hasSource(Source.SciencesDivision) ? (
              <button
                ...
                className={
                  'btn btn-info btn-sm p-2 text-center ' +
                  (tab === StarfleetTrackTab.Other ? 'active' : '')
                }
                onClick={() => setTab(StarfleetTrackTab.Other)}
              >
                {t('EducationPage.starfleet.other')}
              </button>
            ) : undefined}
          </div>
        ) : undefined}
        ...
        {tab !== StarfleetTrackTab.Other ? (
          <div className="my-4">
            <Button size="sm" className="me-3" onClick={() => rollTrack()}>
              <>
                <img
