...
const determinePrefix = (starship: Starship) => {
  if (starship.type === CharacterType.Starfleet) {
    ...
    starship.type === CharacterType.Civilian &&
    !isKlingonWarriorType(starship.spaceframeModel?.type)
  ) {
    return 'SS ';
  } else {
    return '';
...
export const starshipGenerator = (config: IStarshipConfiguration) => {
  ...
  const frames = SpaceframeHelper.instance()
    .getSpaceframes(result, false)
    .filter((s) =>
      result.type === CharacterType.Civilian
        ? true
        : !s.isCivilian && !s.isSmallCraft,
    );
  ...
  if (frames?.length) {
    ...
    result.spaceframeModel = SpaceframeModel.createCustomSpaceframe(
      convertStarshipType(config.type),
      result.serviceYear,
      shuffle(
        PointAllocator.allocatePointsRandomly(
    ...
    if (customClassNames[result.type]?.length) {
      ...
      result.spaceframeModel.name =
        names[Math.floor(Math.random() * names.length)] + ' Class';
    }
  ...
  if (missionProfiles?.length) {
    result.missionProfileStep = new MissionProfileStep(
      missionProfiles[Math.floor(Math.random() * missionProfiles.length)],
    );

    const missionProfileTalents =
      result.missionProfileStep?.type?.talents?.filter(
        (t) =>
          result.spaceframeModel == null ||
  ...
  if (result.version > 1) {
    if (result.type === CharacterType.Starfleet && D20.roll() > 12) {
      ...
      if (
        ...
        result.serviceYear - result.spaceframeModel.serviceYear > 20
      ) {
        result.serviceRecordStep = new ServiceRecordStep(record);
        if (record.type === ServiceRecord.SurvivorOfX) {
          result.serviceRecordStep.selection = randomStarshipEvent(
            result.spaceframeModel,
            result.serviceYear,
          );
        }
  ...
  if (result.type === CharacterType.Starfleet) {
    result.registry = RegistryNumber.generate(
      result.serviceYear,
      result.type,
      result.spaceframeModel,
    );
  ...
  result.name =
    determinePrefix(result) +
    StarshipRandomNameTable(
      config.era,
      config.type,
      result.spaceframeModel?.type,
      result.spaceframeModel?.id,
    );
