...
> = ({ starship, workflow }) => {
  ...
  const getTalents = () => {
    ...
    starship?.missionProfileStep?.type?.talents?.forEach((t) => {
      ...
      if (!talent.isSourcePrerequisiteFulfilled(starship)) {
        ...
        talents.push(
          new RankedTalent(talent, starship.getRankForTalent(talent.name) + 1),
        );
      }
    });
    return talents;
  ...
  const nextPage = () => {
    if (starship.missionProfileStep?.talent == null) {
      ...
      starship.version > 1 &&
      starship.missionProfileStep?.system == null
    ) {
  ...
  const onSelectSystem = (system: System) => {
    store.dispatch(
      setStarshipMissionProfile(starship.missionProfileStep?.type, system),
    );
  };
  ...
  const renderSingleSystem = (system: System) => {
    return (
      <AttributeView
        name={t(makeKey('Construct.system.', System[system]))}
        points={1}
        value={starship.systems[system]}
      />
    );
  };
  ...
  const renderSystemChoice = () => {
    return (
      <>
        <ReactMarkdown>
          {t('MissionProfileTalentSelection.instruction.system')}
        </ReactMarkdown>
  ...
  return (
    <div className="page container ms-0">
      ...
      <Header>
        {starship.version === 1
          ? t('Page.title.missionProfileTalentSelection')
          : t('Page.title.missionProfileDetails')}
      </Header>
        ...
        <>
          <div className="row">
            <div className="col-12 col-md-6">
              ...
              <ReactMarkdown>
                {starship.missionProfileStep?.type?.localizedDescription}
              </ReactMarkdown>
            </div>
            <div className="col-12 col-md-6">
              <Header level={2} className="mt-4">
              ...
              <div className="row row-cols-1 row-cols-lg-3 mt-3">
                <StatView
                  ...
                  value={
                    starship.missionProfileStep?.type?.departments[
                      Department.Command
                    ]
                  }
                  className="col mb-2"
              ...
              {starship.missionProfileStep?.type?.systems?.length === 1
                ? renderSingleSystem(
                    starship.missionProfileStep.type.systems[0],
                  )
                : renderSystemChoice()}
            </div>
          </div>
      ...
      <ReactMarkdown>
        {t('MissionProfileTalentSelection.instruction')}
      </ReactMarkdown>
