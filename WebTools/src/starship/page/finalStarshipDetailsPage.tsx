...
> = ({ starship }) => {
    ...
    if (starship.type === CharacterType.Starfleet) {
      return (
        <div className="mt-5">
          ...
          <ReactMarkdown>
            {t('FinalStarshipDetails.registry.instruction')}
          </ReactMarkdown>
          <div className="d-sm-flex align-items-stretch">
            <label className="textinput-label">
              {t('Construct.other.registry')}
            </label>
            <input
              type="text"
              onChange={(ev) =>
                store.dispatch(setStarshipRegistry(ev.target.value))
              }
      ...
      import(/* webpackChunkName: 'export' */ '../../exportpdf/sheets').then(
        ({ CharacterSheetRegistry }) => {
          ...
          CharacterSheetDialog.show(
            CharacterSheetRegistry.getStarshipSheets(starship),
            'starship',
            starship,
          );
        },
  ...
  useEffect(() => {
    if (
      starship.type === CharacterType.Starfleet &&
        ...
        <div className="col-lg-6 mt-3 mb-4">
          ...
          <div className="d-sm-flex align-items-stretch">
            ...
            <input
              type="text"
              onChange={(ev) =>
                store.dispatch(setStarshipName(ev.target.value))
              }
              value={starship.name}
            />
          </div>
