...
import { RandomStarshipCharacterType } from '../model/randomStarshipCharacterType';
import { RandomStarshipCharacterTypes } from '../model/randomStarshipCharacterTypes';
      ...
      <div id="app">
        <div className="page container ms-0">
          ...
          <main>
            ...
            <ReactMarkdown>
              {t('RandomStarshipPage.instructions')}
            </ReactMarkdown>
            <div className="row">
              <div className="col-md-6 mt-4">
                ...
                <div className="my-4">
                  <DropDownSelect
                    items={RandomStarshipCharacterTypes.instance.types.map(
                      (t) => new DropDownElement(t.type, t.localizedName),
                    )}
                    defaultValue={type ?? ''}
                    onChange={(type) =>
                      setType(type as RandomStarshipCharacterType)
                    }
                  />
                </div>
              </div>
              <div className="col-md-6 mt-4">
                <Header level={2}>
                  {t('StarshipTypeSelection.campaignYear')}
                </Header>

                <div className="my-4">
                  <ServiceYearSelector
                    campaignYear={campaignYear}
                    onChange={(year) => setCampaignYear(year)}
                  />
                </div>
              </div>
            </div>
