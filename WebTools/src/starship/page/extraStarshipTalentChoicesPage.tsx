...
> = ({ starship, workflow }) => {
  ...
  if (
    starship.spaceframeStep.model.talents.filter(
      (t) => t.name === TALENT_NAME_DEDICATED_PERSONNEL,
    )?.length
  ) {
    defaultTalent = new SelectedTalent(TALENT_NAME_DEDICATED_PERSONNEL);
  } else if (
    starship.spaceframeStep.model.talents.filter(
      (t) => t.name === TALENT_NAME_REDUNDANT_SYSTEMS,
    )?.length
  ) {
  ...
  const [selection, setSelection] = useState(
    starship.spaceframeStep.talents[0] ?? defaultTalent,
  );

  const onNext = () => {
    if (
      selection.name === TALENT_NAME_REDUNDANT_SYSTEMS &&
      selection.system == null
    ) {
      Dialog.show(t('Common.error.system'));
    } else if (
      selection.name === TALENT_NAME_DEDICATED_PERSONNEL &&
