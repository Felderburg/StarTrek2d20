...
class SmallCraftStatsPage extends BaseSimpleStarshipPage {
  ...
  getSystemsPoints() {
    return BuildPoints.systemPointsForType(
      this.props.starship.buildType,
      this.props.starship.serviceYear,
      this.props.starship.type,
      this.props.starship.scale,
      this.props.starship.version,
    );
  }
  ...
  renderSystemsText() {
    return (
      <p>
        You have {this.getSystemsPoints()} System Points to distribute to the
        ship's systems.
      </p>
    );
  }
  ...
  renderDepartmentText() {
    return (
      <p>
        You have {this.getDepartmentPoints()} Department Points to distribute to
  ...
  sumTotalDepartments() {
    ...
    DepartmentsHelper.instance
      .getDepartments()
      .forEach((d) => (total += this.props.starship.departments[d]));
    return total;
  }

  canIncreaseDepartment(department: Department) {
    return (
      this.sumTotalDepartments() < this.getDepartmentPoints() &&
      super.canIncreaseDepartment(department)
    );
  }
  ...
  sumTotalSystems() {
    let total = 0;
    allSystems().forEach(
      (s) => (total += this.props.starship.getSystemValue(s)),
    );
    return total;
  }
  ...
  canIncreaseSystem(system: System) {
    return (
      this.sumTotalSystems() < this.getSystemsPoints() &&
      super.canIncreaseSystem(system)
    );
  }
  ...
  renderAppearance() {
    ...
    if (this.isAppearanceSupported()) {
      return (
        <div className="my-4">
          <Header className="mb-4" level={2}>
            {t('Construct.other.appearance')}
          </Header>

          <DropDownSelect
            items={this.getAppearanceOptions()}
