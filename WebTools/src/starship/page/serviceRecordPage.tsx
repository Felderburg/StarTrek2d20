...
const ServiceRecordPageBase: React.FC<IServiceRecordPageProperties> = ({
  ...
  const nextPage = () => {
    ...
    if (starship.serviceRecordStep?.type?.type === ServiceRecord.MajorRefit) {
      ...
      ].includes(starship.serviceRecordStep?.type?.type)
    ) {
      if (starship.serviceRecordStep?.system == null) {
        ok = false;
        Dialog.show(t('Common.error.system'));
  ...
  const onExtraDetailChange = (selection?: string | System) => {
    if (starship.serviceRecordStep) {
      store.dispatch(
        setStarshipServiceRecord(
          starship.serviceRecordStep.type,
          starship.serviceRecordStep.specialRule,
          selection,
  ...
  const rows = serviceRecords.map((r, i) => {
    ...
    return (
      <tbody key={i}>
        <tr>
          <td className="">
            <div className="selection-header">{r.name}</div>
          </td>
          ...
          <tr>
            ...
            <td>
              <div className="markdown-sm">
                <p>
                  <strong>
                    {starship.serviceRecordStep.specialRule.localizedName}
                  </strong>
                </p>
              </div>
              <Markdown className="markdown-sm">
                {starship.serviceRecordStep.specialRule.localizedDescription2e}
              </Markdown>
            </td>
            <td></td>
          </tr>
        ...
        {starship.serviceRecordStep?.type?.type ===
          ServiceRecord.BroughtOutOfMothballs &&
                ...
                <div className="row">
                  <div className="col-12 col-md-6">
                    ...
                    <SimpleStringSelector
                      ...
                      isChecked={(v) =>
                        v === starship.serviceRecordStep?.removedTalent
                      }
