...
const StarshipTalentsPage: React.FC<ISimpleStarshipPageProperties> = ({
  ...
  const nextPage = () => {
    ...
    for (
      let i = 0;
      i < starship.additionalTalents?.length && message == null;
      i++
    ) {
      message = determineSelectedTalentExtraErrors(
        starship.additionalTalents[i],
        starship,
      );
    }
    ...
    if (starship.freeTalentSlots > starship.additionalTalents.length) {
      Dialog.show(
        'Please select ' +
          starship.freeTalentSlots +
          (starship.freeTalentSlots === 1 ? ' talent ' : ' talents ') +
          ' before proceeding.',
      );
    } else if (message) {
      Dialog.show(message);
    ...
    <div className="page container ms-0">
      ...
      <ReactMarkdown>
        {t('StarshipTalentSelection.instruction_one', {
          count: starship.freeTalentSlots,
        })}
      </ReactMarkdown>
