...
const CustomSpaceframeView: React.FC<IStarshipProperties> = ({starship}) => {
    ...
    const getCurrentSystemValuesSortedMaxToMin = () => {
        let result = [];
        allSystems().forEach(s => result.push(new SystemValue(s, starship.systems[s])));
    ...
    const canIncreaseSystem = (system: System) => {
        return (starship.spaceframeModel?.sumSystemPoints ?? 0) < starship.totalAvailableSystemPoints;
    }
    ...
    const sumTotalDepartments = () => {
        let total = 0;
        DepartmentsHelper.instance.getDepartments().forEach(d => total += starship.departments[d]);
        return total;
    }
    ...
    const canIncreaseDepartment = (department: Department) => {
        return sumTotalDepartments() < starship.totalAvailableDepartmentPoints && getDepartment(department) < 5;
    }
    ...
    const isAppearanceSupported = () => {
        return SpaceframeAppearanceModel.getAllAppearanceModels(starship.type, starship.era)?.length;
    }

    const getAppearanceOptions = () => {
        let result = [ new DropDownElement("", "")];
        result.push(...SpaceframeAppearanceModel.getAllAppearanceModels(starship.type, starship.era)
    ...
    if (starship.spaceframeStep?.model != null && starship.spaceframeStep?.model?.isCustom) {
        return (<>
            <section className="my-4 row row-cols-1 row-cols-lg-2">
                <div className="col mb-3">
                    ...
                    <div className="stats-row pt-2">
                        <StatControl statName={t('Construct.other.scale')} value={starship.scale}
                            showIncrease={canIncreaseScale()} showDecrease={canDecreaseScale()}
                            onIncrease={() => { setScale(1) }} onDecrease={() => {setScale(-1)}} />
                    </div>
                </div>
                ...
                <div className="col mb-4">
                    ...
                    <InputFieldAndLabel type="number" value={starship?.spaceframeModel?.serviceYear?.toString() || ""}
                ...
                {isAppearanceSupported()
                    ? (<div className="col mb-4">
                        ...
                        <DropDownSelect items={getAppearanceOptions()}
            ...
            <section className="my-5">
                ...
                <Markdown>{t("CustomSpaceframe.systems.instruction", { "points": starship.totalAvailableSystemPoints})}</Markdown>
            ...
            <section className="my-5">
                ...
                <Markdown>{t("CustomSpaceframe.departments.instruction", { "points": starship.totalAvailableDepartmentPoints})}</Markdown>
