...
const StationTalentsPage: React.FC<IStationPageProperties> = ({ station }) => {
  ...
  const nextPage = () => {
    ...
    for (
      let i = 0;
      i < station.additionalTalents?.length && message == null;
      i++
    ) {
      message = determineSelectedTalentExtraErrors(
        station.additionalTalents[i],
        station,
      );
    }
    ...
    if (station.freeTalentSlots > station.additionalTalents.length) {
      Dialog.show(
        'Please select ' +
          station.freeTalentSlots +
          (station.freeTalentSlots === 1 ? ' talent ' : ' talents ') +
          ' before proceeding.',
      );
    } else if (message) {
      Dialog.show(message);
  ...
  return (
    <LcarsFrame activePage={PageIdentity.StationTalents}>
      <div id="app">
        <StationBreadcrumbs
          pageIdentity={PageIdentity.StationTalents}
          station={station}
        />
        <main className="page container ms-0">
          <Header>{t('Page.title.stationTalents')}</Header>
          <Markdown>
            {t('StarshipTalentSelection.instruction_one', {
              count: station.freeTalentSlots,
            })}
          </Markdown>
          {station.freeTalentSlots > 0 ? (
            <MultiTalentSelectionView
              ...
              talents={talentList()}
              construct={station}
              onSelection={(talent, selectedTalent) =>
                updateSelectedTalent(talent, selectedTalent)
              }
            />
          ) : null}
