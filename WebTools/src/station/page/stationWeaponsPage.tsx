...
const StationWeaponsPage: React.FC<IStationPageProperties> = ({station}) => {
    ...
    const renderWeapons = () => {
        if (station.weapons.length === 0) {
            ...
            return (<tbody>
                {station.weapons.map((w, i) => (<tr key={'weapon-' + i}>
    ...
    const modalContents = () => {
        return (<AddWeaponView onClose={() => closeModal()}
            addWeapon={(weapon) => store.dispatch(addStationWeapon(weapon))}
            version={station.version}
            mode={station.isMineLayer ? AddWeaponMode.IncludeMines : AddWeaponMode.NoMines} />)
    }
                    ...
                    <table className="selection-list">
                        <thead>
                            <tr>
                                ...
                                <th>{t(station.version === 1 ? 'Weapon.common.dice' : 'Weapon.common.severity')}</th>
