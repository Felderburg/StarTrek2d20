...
export const ScaleSelector: React.FC<IScaleSelectorProperties> = ({scale, onChange}) => {
    const { t } = useTranslation();

    return (<>
            <div className="d-sm-flex align-items-stretch">
                <label htmlFor="scale" className="textinput-label">{t('Construct.other.scale')}</label>
                <input
                    style={{width: "4rem"}}
                    id="scale"
                    type="number"
