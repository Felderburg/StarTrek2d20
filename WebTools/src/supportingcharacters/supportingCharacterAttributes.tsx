...
const SupportingCharacterAttributes: React.FC<ICharacterProperties> = ({
  ...
  const toggleSpeciesAttribute = (attribute: Attribute) => {
    if (character?.speciesStep?.attributes?.includes(attribute)) {
      ...
      store.dispatch(
        modifyCharacterAttribute(attribute, StepContext.Species, true),
      );
    }
  };
  ...
  const attributes = AttributesHelper.getAllAttributes().map((a, i) => {
    ...
    if (
      character?.speciesStep?.species === Species.Custom ||
      speciesModel?.isAttributeSelectionRequired
    ) {
    ...
    } else if (speciesModel?.id !== Species.Ktarian) {
      // most species
      let deltaValue =
        character.speciesStep?.attributes?.filter((at) => at === a)?.length ??
        0;
      deltaValue -=
        character.speciesStep?.decrementAttributes?.filter((at) => at === a)
          ?.length ?? 0;
      return (
        <tr key={i}>
          <td className="selection-header">
      ...
      // Ktarians have two attributes pre-defined, and can choose from Secondary Attributes as a third attribute
      ...
      return (
        <tr key={i}>
          ...
          {checkBox}
          <td className="text-center">
            {speciesHasAttribute || isChecked ? '+1' : '-'}
          </td>
          <td className="text-center">
            {val + (speciesHasAttribute || isChecked ? 1 : 0)}
          </td>
        </tr>
      );
    }
