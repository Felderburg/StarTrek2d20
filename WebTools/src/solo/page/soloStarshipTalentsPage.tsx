...
const SoloStarshipTalentsPageBase: React.FC<ISoloStarshipTalentsProperties> = ({
  ...
  const navigateToNextPage = () => {
    if (starship.additionalTalents?.length !== starship.scale) {
      Dialog.show(
        t('StarshipTalentSelection.instruction', {
          count: starship.scale,
  ...
  const talents = TalentsHelper.getTalents()
    ...
    .sort((t1, t2) =>
      t1
        .localizedNameForSource(Source.CaptainsLog)
        .localeCompare(t2.localizedNameForSource(Source.CaptainsLog)),
    );
    ...
    return (
      <tr key={i}>
        <td className="selection-header-small">
          {t.localizedNameForSource(Source.CaptainsLog)}
        </td>
        <td>{lines}</td>
        <td>
          <CheckBox
