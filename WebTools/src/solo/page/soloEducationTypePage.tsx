...
const SoloEducationTypePage: React.FC<ICharacterProperties> = ({
  ...
  const [randomType, setRandomType] = useState(
    character?.educationStep ? character.type : null,
  );
  ...
  const toTableRow = (type: CharacterTypeModel, i: number) => {
    return (
      <tr
        key={i}
        onClick={() => {
          if (Window.isCompact()) typeSelected(type.type);
        }}
      >
  ...
  const types =
    randomType != null
      ...
      : CharacterTypeModel.getSoloCharacterTypes().map((e, i) =>
          toTableRow(e, i),
        );
  ...
  return (
    <div className="page container ms-0">
      ...
      <p className="mt-3">{t('SoloEducationTypePage.instruction')}</p>
      <div className="my-4">
        <Button
          size="sm"
          className="me-3"
