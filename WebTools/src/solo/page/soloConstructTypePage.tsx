...
const SoloConstructTypePage = () => {
  ...
  const createSoloStarship = () => {
    store.dispatch(
      createStarship(Starship.createSoloStarship(store.getState().context.era)),
    );
    ...
    <div className="page">
      <div className="container ms-0">
        ...
        <div className="button-column">
          <Button className="mt-4" onClick={() => createCharacter()}>
            {t('SoloConstructTypePage.character')}
          </Button>
          <Button className="mt-4" onClick={() => createSoloStarship()}>
            {t('SoloConstructTypePage.starship')}
          </Button>
        </div>
      </div>
    </div>
