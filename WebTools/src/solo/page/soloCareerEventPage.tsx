...
const SoloCareerEventPageBase: React.FC<ISoloCareerEventProperties> = ({
  ...
  const careerEventSelected = (careerEvent: CareerEventModel) => {
    ...
    Navigation.navigateToPage(
      context === StepContext.CareerEvent2
        ? PageIdentity.SoloCareerEventDetails2
        : PageIdentity.SoloCareerEventDetails1,
    );
  };
  ...
  const toTableRow = (careerEvent: CareerEventModel, i: number) => {
    ...
    const disciplines = careerEvent.disciplines.map((d, i) => {
      return (
        <div key={i}>{DepartmentsHelper.instance.getDepartmentName(d)}</div>
      );
    });
  ...
  const events =
    randomEvent != null
      ...
      : CareerEventsHelper.getSoloCareerEvents().map((c, i) =>
          toTableRow(c, i),
        );
  ...
  return (
    <div className="page container ms-0">
      <SoloCharacterBreadcrumbs
        pageIdentity={
          context === StepContext.CareerEvent1
            ? PageIdentity.CareerEvent1
            : PageIdentity.CareerEvent2
        }
      />
      ...
      <div className="my-4">
        <Button
          ...
          className="me-3"
          onClick={() =>
            setRandomEvent(CareerEventsHelper.generateEvent(character).roll)
          }
        >
          <>
            <img
