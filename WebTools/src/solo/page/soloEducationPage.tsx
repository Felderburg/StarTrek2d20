...
const SoloEducationPage: React.FC<ICharacterProperties> = ({ character }) => {
  ...
  const toTableRow = (track: TrackModel, i: number) => {
    return (
      <tr
        key={i}
        onClick={() => {
          if (Window.isCompact()) trackSelected(track);
        }}
      >
  ...
  const types =
    randomTrack != null
      ...
      : TracksHelper.instance
          .getSoloTracks(character.type)
          .map((e, i) => toTableRow(e, i));
  ...
  return (
    <div className="page container ms-0">
      ...
      <Header>
        {t(makeKey('SoloEducationPage.type.', CharacterType[character.type]))}
      </Header>
      <p className="mt-3">
        {t(
          makeKey(
            'SoloEducationPage.instruction.',
