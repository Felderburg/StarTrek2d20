...
export const ValueRandomTable = (species?: Species, skill?: Department) => {
  ...
  if (roll <= 15 && hasSource(Source.ContinuingMissions)) {
    ...
    if (subRoll <= 10) {
      if (
        species != null &&
        speciesRandomValues[species] != null &&
        skill != null &&
        disciplineRandomValues[skill] != null
      ) {
        const table =
          D20.roll() <= 10
            ? speciesRandomValues[species]
            : disciplineRandomValues[skill];
        return table();
...
export const randomUniqueValue = (
