...
export class TokenSvgBuilder {
  static async loadDependenciesAndCreateSvg(
    token: TokenModel,
    rounded: boolean = false,
    fancyBorder: boolean = false,
  ) {
    ...
    if (
      SpeciesRestrictions.isRubberHeaded(token.species) &&
      !HeadCatalog.instance.isRubberHeadExtensionLoaded
    ) {
    ...
    return (
      `<?xml version="1.0" encoding="UTF-8"?>
            <svg viewBox="0 0 400 400" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                <title>Token</title>
                <defs>` +
      (token.uniformEra === UniformEra.Enterprise
        ? EnterpriseEraStarfleetLogoGradient
        : '') +
      `<clipPath id="roundClipPath">
                        <circle cx="200px" cy="200px" r="200px" fill="#ffffff" />
                    </clipPath>` +
      RankIndicatorCatalog.instance.getBorderRankDefinitions(
        token,
        fancyBorder,
      ) +
      ...
      ProstheticCatalog.instance.getProsthetic(
        token,
        ProstheticPlacement.BaseHead,
      ) +
      NasoLabialFoldCatalog.instance.getNasoLabialFold(token) +
      FacialHairCatalog.instance.getFacialHair(
        token,
      ...
      ExtrasCatalog.instance.getExtras(
        token,
        ExtraCategory.Face,
        true,
        token.hairType,
  ...
  private static createBorder(token: TokenModel) {
    ...
    return (
      '<circle cx="200px" cy="200px" r="190px" stroke="' +
      colour +
      '" stroke-width="20px" />' +
  ...
  private static getBorderColor(token: TokenModel) {
    return UniformPackCollection.instance
      .getUniformPack(token.uniformEra)
