...
const ExtraSelectionView: React.FC<ITokenPageProperties> = ({ token }) => {
  ...
  const isExtraCategoryPresent = (category: ExtraCategory) => {
    return (
      token.extras.filter((i) =>
        ExtrasCatalog.instance.isInCategory(i, category),
      ).length > 0
    );
  };
  ...
  return (
    <>
      ...
      <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
        {ExtrasCatalog.instance
          .getSwatches(token, ExtraCategory.Ear)
          .map((s) => (
            <SwatchButton
              svg={s.svg}
              title={s.localizedName}
              onClick={() => addExtra(s.id, ExtraCategory.Ear)}
              active={
                token.extras.indexOf(s.id) >= 0 ||
                (s.id === ExtraType.None &&
                  !isExtraCategoryPresent(ExtraCategory.Ear))
              }
              token={token}
      ...
      <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
        {ExtrasCatalog.instance
          .getSwatches(token, ExtraCategory.Forehead)
          .map((s) => (
            <SwatchButton
              svg={s.svg}
              title={s.localizedName}
              ...
              active={
                token.extras.indexOf(s.id) >= 0 ||
                (s.id === ExtraType.None &&
                  !isExtraCategoryPresent(ExtraCategory.Forehead))
              }
              token={token}
              size={token.species === Species.Ferengi ? 'lg' : 'md'}
      ...
      <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
        {ExtrasCatalog.instance
          .getSwatches(token, ExtraCategory.Face)
          .map((s) => (
            <SwatchButton
              svg={s.svg}
              title={s.localizedName}
              ...
              active={
                token.extras.indexOf(s.id) >= 0 ||
                (s.id === ExtraType.None &&
                  !isExtraCategoryPresent(ExtraCategory.Face))
              }
              token={token}
              size="lg"
      ...
      {token.primarySpecies === Species.LiberatedBorg ? (
        <>
          <p className="mt-4">
            {t('TokenCreator.section.extras.borgImplants')}:
          </p>
          <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
            {ExtrasCatalog.instance
              .getSwatches(token, ExtraCategory.BorgImplant)
              .map((s) => (
                <SwatchButton
                  svg={s.svg}
                  title={s.localizedName}
                  ...
                  active={
                    token.extras.indexOf(s.id) >= 0 ||
                    (s.id === ExtraType.None &&
                      !isExtraCategoryPresent(ExtraCategory.BorgImplant))
                  }
                  token={token}
                  size="lg"
                  key={'extra-swatch-headwear-' + s.id}
      ...
      <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
        {ExtrasCatalog.instance
          .getSwatches(token, ExtraCategory.Headwear)
          .map((s) => (
            <SwatchButton
              svg={s.svg}
              title={s.localizedName}
              ...
              active={
                token.extras.indexOf(s.id) >= 0 ||
                (s.id === ExtraType.None &&
                  !isExtraCategoryPresent(ExtraCategory.Headwear))
              }
              token={token}
              size="lg"
              key={'extra-swatch-headwear-' + s.id}
