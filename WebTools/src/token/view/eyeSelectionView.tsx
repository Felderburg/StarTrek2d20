...
const EyeSelectionView: React.FC<ITokenPageProperties> = ({ token }) => {
  ...
  return (
    <>
      ...
      <ColorSelection
        colors={SpeciesRestrictions.getEyeColors(token.species)}
        onSelection={(c) => store.dispatch(setTokenEyeColor(c))}
      />
      ...
      {SpeciesRestrictions.isRubberHeaded(token.species) ? null : (
        <>
          ...
          <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
            {EyeCatalog.instance.getSwatches(token).map((s) => (
              <SwatchButton
                svg={s.svg}
                title={s.name}
                onClick={() => store.dispatch(setTokenEyeType(s.id))}
                active={token.eyeType === s.id}
                token={token}
                key={'eye-swatch-' + s.id}
              />
            ))}
