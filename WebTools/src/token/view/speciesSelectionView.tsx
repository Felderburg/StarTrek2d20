      ...
      return (
        <>
          ...
          <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
            {SpeciesOptionCatalog.instance.getSwatches(token).map((s) => (
              <SwatchButton
                svg={s.svg}
                title={s.name}
                size="lg"
                onClick={() => store.dispatch(setTokenSpeciesOption(s.id))}
                active={token.speciesOption === s.id}
                token={token}
    ...
    return speciesList()
      .map(
        (s) =>
          new DropDownElement(
            s,
            SpeciesHelper.getSpeciesByType(s).localizedName,
          ),
      )
  ...
  if (isLoading) {
    ...
    return (
      <div className="mt-4">
        ...
        <DropDownSelect
          items={speciesListAsOption()}
          defaultValue={token.primarySpecies}
          onChange={(s) => selectSpecies(s as Species)}
          id="species"
        />
        ...
        {token.primarySpecies === Species.LiberatedBorg ? (
          <>
            ...
            <DropDownSelect
              className="mb-3"
              items={secondarySpeciesListAsOption()}
              defaultValue={token.secondarySpecies ?? Species.Human}
              onChange={(s) => selectSecondarySpecies(s as Species)}
              id="secondarySpecies"
            />
          </>
        ) : undefined}
