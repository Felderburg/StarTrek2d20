    ...
    return (
      <>
        ...
        <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
          {NoseCatalog.instance.getSwatches(token).map((s) => (
            <SwatchButton
              svg={s.svg}
              title={s.localizedName}
              onClick={() => store.dispatch(setTokenNoseType(s.id))}
              active={token.noseType === s.id}
              token={token}
        ...
        <div className="d-flex flex-wrap" style={{ gap: '0.5rem' }}>
          {NasoLabialFoldCatalog.instance.swatches.map((s) => (
            <SwatchButton
              svg={s.svg}
              title={s.localizedName}
              onClick={() => store.dispatch(setTokenNasoLabialFoldType(s.id))}
              active={token.nasoLabialFold === s.id}
              token={token}
