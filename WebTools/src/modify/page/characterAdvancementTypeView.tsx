...
export const CharacterAdvancementTypeView: React.FC<
  ICharacterAdvancementTypeViewProperties
> = ({
  character,
  onNextStep,
  onPreviousStep,
  ...
  const [choice, setChoice] = useState<CharacterAdvancementChoice | undefined>(
    undefined,
  );
  const [attributeSelection, setAttributeSelection] = useState<
    Attribute | undefined
  >(undefined);
  const [removeAttributeSelection, setRemoveAttributeSelection] = useState<
    Attribute | undefined
  >(undefined);
  const [departmentSelection, setDepartmentSelection] = useState<
    Department | undefined
  >(undefined);
  const [removeDepartmentSelection, setRemoveDepartmentSelection] = useState<
    Department | undefined
  >(undefined);
  const [focusSelection, setFocusSelection] = useState<string | undefined>(
    undefined,
  );
  const [removeFocusSelection, setRemoveFocusSelection] = useState<
    string | undefined
  >(undefined);
  ...
  >(undefined);
  const [removeTalentSelectionIndex, setRemoveTalentSelectionIndex] = useState<
    number | undefined
  >(undefined);
  ...
  const dropDownChoices = () => {
    ...
    result.push(
      new DropDownElement(
        CharacterAdvancementChoice.Attribute,
    ...
    result.push(
      new DropDownElement(
        ...
        t('Construct.other.department'),
      ),
    );
    if (type !== CharacterAdvancementType.CharacterArc) {
      result.push(
        new DropDownElement(
          CharacterAdvancementChoice.Focus,
    ...
    if (type === CharacterAdvancementType.CharacterArc) {
      result.push(
        new DropDownElement(
          CharacterAdvancementChoice.Value,
        ...
        if (talentSelection == null || removeTalentSelectionIndex == null) {
          ...
          store.dispatch(
            modifyCharacterAddAdvancement(
              choice,
              talentSelection,
              character.talents[removeTalentSelectionIndex],
              logEntry,
              logEntryCallback,
            ),
          );
  ...
  const talentAdditionalDetailsSelected = () => {
    if (determineSelectedTalentExtraErrors(talentSelection) != null) {
      ...
      talentSelection?.talent === TALENT_NAME_VISIT_EVERY_STAR &&
      !talentSelection.focuses?.length
    ) {
  ...
  const isAttributeIncrementable = (a: Attribute, c: Character) => {
    ...
    if (
      ...
      type !== CharacterAdvancementType.CharacterArc &&
      value >= Character.ABSOLUTE_MAX_ATTRIBUTE - 1
    ) {
      return false;
    } else {
      return true;
  ...
  const isDepartmentIncrementable = (d: Department, c: Character) => {
    ...
    if (
      ...
      type !== CharacterAdvancementType.CharacterArc &&
      value >= Character.ABSOLUTE_MAX_DEPARTMENT - 1
    ) {
      return false;
    } else {
      return true;
  ...
  const handleAdditionalTalentSelections = () => {
    if (talentSelection?.talent === TALENT_NAME_WARRIORS_SPIRIT) {
      ...
      [TALENT_NAME_IM_A_DOCTOR_NOT_A].includes(talentSelection?.talent)
    ) {
      return (
        <div className="col-12 col-md-6">
      ...
      [TALENT_NAME_BOLD, TALENT_NAME_CAUTIOUS].includes(talentSelection?.talent)
    ) {
      return (
        <div className="col-12 col-md-6">
          <BoldOrCautiousDepartmentSelectionView
        ...
        return (
          <div className="row">
            ...
            <div className="col-12 col-md-6">
              ...
              <FocusSelectionView
                addFocus={(f) => setFocusSelection(f)}
    ...
    ModalControl.show(
      ...
      <div>
        <SimpleTalentSelectionList
          construct={character}
          talents={talents}
          onSelection={(t) => setTalentSelection(t == null ? undefined : t)}
        />
  ...
  return (
    <>
      <div className="row">
        <div className="col-12">
          <Markdown className="mt-4">
            {t('CharacterAdvancementType.choice.instruction')}
          </Markdown>
          <div className="my-3">
            <DropDownSelect
              ...
              onChange={(v) =>
                setChoice(
                  v === '' ? undefined : (v as CharacterAdvancementChoice),
                )
              }
              defaultValue={choice ?? ''}
            />
