...
export class GeneratedTngPortraitA4CharacterSheet extends BaseTNGGeneratedCharacterSheet {
  ...
  writeStress(
    ...
    super.writeStress(
      ...
      page,
      character,
      GeneratedTngPortraitA4CharacterSheet.lightPurpleColour,
    );
  }
  ...
  writeTitle(page: PDFPage) {
    ...
    const fontSize = this.determineIdealFontWidth(
      [text],
      GeneratedTngPortraitA4CharacterSheet.headingColumn.width,
      25,
      18,
      this.headingFont,
    );
    ...
    page.drawText(text, {
      x:
        GeneratedTngPortraitA4CharacterSheet.headingColumn.start.x +
        GeneratedTngPortraitA4CharacterSheet.headingColumn.width -
        width,
      ...
      if (handledTalents.includes(talent.name)) {
        ...
        this.fillField(
          ...
          talent.localizedDisplayName +
            ' [Rank ' +
            character.getRankForTalent(t.talent) +
            ']',
        );
      } else {
    ...
    if (character.version > 1 && character.pastime?.length) {
      ...
      character.pastime.forEach((p) => {
        this.fillField(
          form,
          'Focus ' + ++index,
          i18next.t('Construct.other.pastimes') + ': ' + p,
        );
      });
    }
  }
