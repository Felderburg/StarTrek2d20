...
export class GmTrackerPdfSheet {
  ...
  async initializeFonts(pdf: PDFDocument) {
    ...
    const baseFontBytes = await fetch(
  ...
  async populate(pdf: PDFDocument, characters: Character[]) {
    ...
    decorations.writeName(
      page,
      i18next.t('GMTracker.title'),
      tealColour2e,
      this.headingFont,
      new Column(75, 48, 15, 550 - 75),
    );
    ...
    for (let i = 0; i < characters.length; i++) {
      ...
      if (
        ...
        c.npcGenerationStep?.type != null
      ) {
        name +=
          ' (' +
          i18next
            .t(makeKey('NpcType.', NpcType[c.npcGenerationStep?.type], '.name'))
            .toLocaleUpperCase() +
          ')';
      }
      ...
      if (c.isStressTrackPresent) {
        ...
        column1 = this.writeStressBoxes(
          new PageArea(column1, page),
          pdf.getForm(),
          c.stress,
          i,
        );
      ...
      if (column1 != null) {
        ...
        if (c.talents?.length) {
          paragraph?.append(
            i18next.t('Construct.other.talents').toLocaleUpperCase() + ': ',
            new FontOptions(9, FontType.Bold),
            tealColour2e,
          );
          paragraph?.append(
            c.talents.map((t) => t.displayName).join(', '),
            new FontOptions(9),
          );
          paragraph?.write();
          paragraph = paragraph?.nextParagraph();
  ...
  writeSubTitle(area: PageArea, text: string) {
    ...
    area.page.drawLine({
      start: {
        x: block.start.x,
        y: area.page.getHeight() - (y + 3 - block.height / 2),
      },
      end: {
