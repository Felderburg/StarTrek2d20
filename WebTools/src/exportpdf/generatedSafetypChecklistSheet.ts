...
export class GeneratedSafetyChecklistSheet extends BaseNonForm2eSheet {
    ...
    firstColumn(additionalPage: PDFPage, pdf: PDFDocument) {
        const column2 = new Column(538.5 - (284.7-56.7), 75, 715-75, 284.7-56.7);
        return new Column(56.7, 75, 715-75, 284.7-56.7, column2);
    }

    async populateEvaluation(pdf: PDFDocument, evaluation: {[category: string]: SafetyEvaluationType}) {
        await this.initializeFonts(pdf);
        let page = pdf.getPage(0);

        new PortraitSheetDecorations().drawSheetDecorations(page, tealColour2e);
        this.writeName(page, i18next.t("Page.title.safetyChecklist"), tealColour2e, this.headingFont, this.nameColumn);
        ...
        SafetySections.instance.sections.forEach(section => {
            ...
            heading.append(section.localizedName.toLocaleUpperCase(), new FontSpecification(this.headingFont, 9));
            heading.write();
    ...
    writeAllCategories(page: PDFPage, section: SafetySection,
            evaluation: {[category: string]: SafetyEvaluationType}, column: Column) {

        const evaluationColumnWidth = 30;
        let th1 = new Paragraph(page, new Column(column.start.x + column.width - 3 * evaluationColumnWidth,
            column.start.y, 20, 30), this.fonts);
        let th2 = new Paragraph(page, new Column(column.start.x + column.width - 2 * evaluationColumnWidth,
            column.start.y, 20, 30), this.fonts);
        let th3 = new Paragraph(page, new Column(column.start.x + column.width - 1 * evaluationColumnWidth,
            column.start.y, 20, 30), this.fonts);
        th1.textAlignment = TextAlign.Centre;
        th2.textAlignment = TextAlign.Centre;
        th3.textAlignment = TextAlign.Centre;

        th1.append(i18next.t('SafetyEvaluationType.alwaysOk'), new FontOptions(8, FontType.Bold));
        th2.append(i18next.t('SafetyEvaluationType.yellowAlert'), new FontOptions(8, FontType.Bold));
        th3.append(i18next.t('SafetyEvaluationType.redAlert'), new FontOptions(8, FontType.Bold));
        ...
        let paragraphColumn = new Column(column.start.x, column.start.y, column.height,
            columnWidth, column.nextColumnHelper);

        section.categories.forEach(c => {
            let paragraph = new Paragraph(page, paragraphColumn, this.fonts);
            paragraph.append(i18next.t(makeKey("SafetySection.category.", c)), new FontOptions(8));
            paragraph.write();

            let type = evaluation[c];
            switch (type) {
                case SafetyEvaluationType.AlwaysOk: {
                    let tempColumn = new Column(paragraphColumn.start.x + paragraphColumn.width,
                        paragraphColumn.start.y, 20, evaluationColumnWidth);
                    let evaluationParagraph = new Paragraph(page, tempColumn, this.fonts);
                    evaluationParagraph.textAlignment = TextAlign.Centre;
                    evaluationParagraph.append(CHALLENGE_DICE_NOTATION, new FontOptions(8, FontType.Symbol), tealColour2e);
                    evaluationParagraph.write();
                }
                break;
                case SafetyEvaluationType.YellowAlert: {
                    let tempColumn = new Column(paragraphColumn.start.x + paragraphColumn.width + evaluationColumnWidth,
                        paragraphColumn.start.y, 20, evaluationColumnWidth);
                    let evaluationParagraph = new Paragraph(page, tempColumn, this.fonts);
                    evaluationParagraph.textAlignment = TextAlign.Centre;
                    evaluationParagraph.append(CHALLENGE_DICE_NOTATION, new FontOptions(8, FontType.Symbol), goldColour2e);
                    evaluationParagraph.write();
                }
                break;
                case SafetyEvaluationType.RedAlert: {
                    let tempColumn = new Column(paragraphColumn.start.x + paragraphColumn.width + 2 * evaluationColumnWidth,
                        paragraphColumn.start.y, 20, evaluationColumnWidth);
                    let evaluationParagraph = new Paragraph(page, tempColumn, this.fonts);
                    evaluationParagraph.textAlignment = TextAlign.Centre;
                    evaluationParagraph.append(CHALLENGE_DICE_NOTATION, new FontOptions(8, FontType.Symbol), redColour2e);
                    evaluationParagraph.write();
                }
            }
        ...
        return new Column(paragraphColumn.start.x, paragraphColumn.start.y, paragraphColumn.height,
            column.width, paragraphColumn.nextColumnHelper);
    }
