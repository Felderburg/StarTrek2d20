  ...
  describeFully(weapon: Weapon, diceProvider: IWeaponDiceProvider) {
    ...
    if (this.personal) {
      type =
        ...
        weapon.personalWeaponType !== PersonalWeaponType.KeratinDart
    ...
    if (this.version > 1 && weapon.injuryType != null) {
      injuryType =
        i18next.t(makeKey('InjuryType.', InjuryType[weapon.injuryType])) + ' ';
    }

    let qualities = weapon.effectsAndQualities
      .map((q) => q.localizedDescription)
      .join(', ');
    if (qualities?.length) {
      qualities = ', ' + qualities;
    } else {
    ...
    let text =
      ...
      injuryType +
      dice +
      (this.version === 1 ? CHALLENGE_DICE_NOTATION : '') +
      qualities +
      (weapon.hands != null
        ? ', ' + i18next.t('Weapon.common.size', { hands: weapon.hands })
        : '');
    return text;
  }
