...
export class Generated2eStarshipSheet extends BaseNonForm2eSheet {
  ...
  writeStarshipName(page: PDFPage, starship: Starship, colour: SimpleColor) {
    if (starship.name?.length) {
      ...
      this.writeName(
        page,
        i18next.t('ViewPage.unnamedStarship'),
        colour,
        this.headingFont,
        this.nameColumn,
      );
  ...
  async populate(pdf: PDFDocument, construct: Construct) {
    ...
    paragraph.append(
      i18next.t('Construct.other.launchYear').toLocaleUpperCase() + ': ',
      new FontSpecification(this.boldFont, 9),
      colour,
    );
    paragraph.append(
      '' +
        (starship.spaceframeModel?.serviceYear ?? starship.serviceYear ?? ''),
      new FontSpecification(this.textFont, 9),
    );
    paragraph.write();
    ...
    paragraph.append(
      i18next.t('Construct.other.timeline').toLocaleUpperCase() + ': ',
    ...
    if (starship.className?.length && starship.version === 1) {
      ...
      paragraph.append(
        i18next.t('Construct.other.spaceFrame').toLocaleUpperCase() + ': ',
        new FontOptions(9, FontType.Bold),
        colour,
      );
      if (starship.spaceframeModel != null) {
        paragraph.append(
          starship.spaceframeModel?.localizedName,
          new FontOptions(9),
        );
      } else {
