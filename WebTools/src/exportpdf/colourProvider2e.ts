...
export const labelColourProvider = (era: Era, label: string) => {
  if (
    label === 'Construct.discipline.command' ||
    label === 'Construct.discipline.conn' ||
    label === 'Construct.department.command' ||
    label === 'Construct.department.conn'
  ) {
    return [Era.NextGeneration, Era.PicardProdigy, Era.Discovery32].includes(
      era,
    )
      ? redColour2e
      : goldColour2e;
  } else if (
    label === 'Construct.discipline.engineering' ||
    ...
    label === 'Construct.discipline.medicine' ||
