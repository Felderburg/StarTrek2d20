...
export class Landscape2eCreatureSheet extends BaseNonForm2eSheet {
    ...
    static readonly column3 = new Column(216.1 + 216.1 + 55.6 + 20, 72.6, 479.3, 216.1);
    static readonly column2 = new Column(216.1 + 55.6 + 10, 72.6, 479.3, 216.1);
    static readonly column1 = new Column(55.6, 72.6, 479.3, 216.1, Landscape2eCreatureSheet.column2);
    ...
    async populate(pdf: PDFDocument, construct: Construct) {
        ...
        if (column) {
            ...
            await new TalentWriter(page, this.fonts, construct.version, tealColour2e, true)
    ...
    writeDetails(aera: PageArea, creature: Creature) {
        ...
        paragraph?.append(i18next.t("Construct.creature.creatureType").toLocaleUpperCase() + ": ", new FontSpecification(this.boldFont, 9), tealColour2e);
        paragraph?.append(creature.creatureType?.localizedName, new FontSpecification(this.textFont, 9));
        paragraph?.write();

        paragraph = paragraph?.nextParagraph();
        paragraph?.append(i18next.t("Construct.creature.dietType").toLocaleUpperCase() + ": ", new FontSpecification(this.boldFont, 9), tealColour2e);
        paragraph?.append(creature.diet?.localizedName, new FontSpecification(this.textFont, 9));
        paragraph?.write();

        paragraph = paragraph?.nextParagraph();
        paragraph?.append(i18next.t("Construct.creature.size").toLocaleUpperCase() + ": ", new FontSpecification(this.boldFont, 9), tealColour2e);
        paragraph?.append(creature.size?.localizedName, new FontSpecification(this.textFont, 9));
        paragraph?.write();
    ...
    writeTitle(title: string, page: PDFPage, colour: SimpleColor) {
        ...
        const fontSize = determineIdealFontWidth([ text ],
            Landscape2eCreatureSheet.headingColumn.width, 10, 7.5, this.headingFont);
