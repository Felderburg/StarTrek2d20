...
export class SpaceframeModel implements IServiceYearProvider {
  ...
  constructor(
    ...
    additionalTraits: string[] = ['Federation Starship'],
    ...
    this.talents = talents.map((t) =>
      t instanceof SelectedTalent
        ? (t as SelectedTalent)
        : new SelectedTalent((t as TalentModel).name),
    );
  ...
  get isMissionPodAvailable() {
    return (
      this.id === Spaceframe.Nebula ||
      this.id === Spaceframe.Luna ||
      this.hasTalent('Mission Pod')
    );
  }
  ...
  get serviceYearForRefitCalculation() {
    if (
      this.id === Spaceframe.Miranda_2E ||
      this.id === Spaceframe.Miranda_UP
    ) {
      ...
      return this.talents.filter((t) => {
        ...
        model.prerequisites.forEach((p) => {
          if (p instanceof MaxServiceYearPrerequisite) {
            result =
              result && serviceYear <= (p as MaxServiceYearPrerequisite).year;
  ...
  static createStandardSpaceframe(
    id: Spaceframe,
    type: CharacterType,
    name: string,
    serviceYear: number,
  ...
  static createCustomSpaceframe(
    type: CharacterType,
    serviceYear: number,
    systems: number[] = [7, 7, 7, 7, 7, 7],
    ...
    return new SpaceframeModel(
      ...
      '',
      serviceYear,
      [new SourcePrerequisite(Source.None)],
      systems,
      departments,
      scale,
      [],
