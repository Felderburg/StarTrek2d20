...
export class CareerEventModel {
  ...
  constructor(
    ...
    focuses: string[] = [],
    prerequisites: Source | ICharacterPrerequisite[] = [],
  ) {
    this.name = name;
    this.description = description;
    this.attributes = attributes;
    this.disciplines = disciplines;
    this.focusSuggestions = focusSuggestions?.length
      ? focusSuggestions
      : focuses.join(', ');
    ...
    if (Array.isArray(prerequisites)) {
      ...
      this.prerequisites = [
        new SourceCharacterPrerequisite(prerequisites as Source),
      ];
    }
  }
  ...
  isPrerequisiteFulfilled(character: Character) {
    let result = true;
    this.prerequisites.forEach(
      (p) => (result = result && p.isPrerequisiteFulfilled(character)),
    );
    return result;
  }
