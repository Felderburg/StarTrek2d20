  ...
  isPrerequisiteFulfilled(character: Character): boolean {
    if (
      new SourceCharacterPrerequisite(
        Source.KlingonCore,
      ).isPrerequisiteFulfilled(character)
    ) {
      return !new KlingonCharacterPrerequisite().isPrerequisiteFulfilled(
        character,
      );
    } else {
      return true;
...
class ChildPrerequisite implements ICharacterPrerequisite {
  isPrerequisiteFulfilled(character: Character): boolean {
    return character.type === CharacterType.Child || character.age.isChild;
  }
}
...
export class RolesHelper {
  ...
  private _roles: RoleModel[] = [
    ...
    new RoleModel(
      ...
      new SourceCharacterPrerequisite(Source.CommandDivision),
      new NotCharacterPrerequisite(
        new SourceCharacterPrerequisite(Source.Core2ndEdition),
      ),
      new NotKlingonPrerequisite(),
  ...
  getRoles(character: Character) {
    ...
    for (var r of list) {
      if (r.isPrerequisitesFilled(character)) {
        if (
          ...
          (r.id === Role.CommandingOfficer || r.id === Role.Admiral)
        ) {
          continue;
        }
        roles.push(r);
    ...
    let temp = roles.sort((a, b) => {
      if (a.department == null && b.department == null) {
        ...
        a.department != null &&
        b.department != null &&
        departments.indexOf(a.department) >= 0 &&
        departments.indexOf(b.department) >= 0
      ) {
        return a.id - b.id;
      } else if (
        a.department != null &&
