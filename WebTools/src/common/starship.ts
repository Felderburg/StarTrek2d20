...
export const refitCalculator = (starship: Starship) => {
  if (
    starship.buildType === ShipBuildType.Starship &&
    starship?.serviceYear &&
    starship?.spaceframeModel?.serviceYearForRefitCalculation
  ) {
    if (
      starship.version === 1 &&
      starship.serviceYear >= 2400 &&
      starship.spaceframeModel.serviceYearForRefitCalculation >= 2400
    ) {
      return Math.max(
        0,
        Math.floor(
...
export class ShipBuildTypeModel {
  ...
  get localizedName() {
    return i18next.t(
      makeKey('ShipBuildType.', ShipBuildType[this.type], '.name'),
    );
  }
}
...
export class Starship extends Construct implements IWeaponDiceProvider {
  ...
  static createStandardStarship(
    era: Era = Era.NextGeneration,
  ...
  set spaceframeModel(spaceframe: SpaceframeModel) {
    ...
    if (
      original?.model?.isCustom &&
      spaceframe.isCustom &&
      original?.appearance != null
    ) {
  ...
  get isSmallCraft() {
    return (
      this.buildType === ShipBuildType.Pod ||
      this.buildType === ShipBuildType.Shuttlecraft ||
      this.buildType === ShipBuildType.Runabout
    );
  }
  ...
  get defaultTraits() {
    ...
    if (this.missionProfileStep?.type?.traits?.length) {
      this.missionProfileStep.type.traits
        .split(', ')
        .forEach((t) => trait.push(t.trim()));
    }
    return trait;
  }
    ...
    if (this.stereotype === Stereotype.SoloStarship) {
      ...
      if (this.spaceframeModel !== undefined) {
        ...
        this.spaceframeModel
          ...
          .forEach(
            (t) =>
              (numTalents += t.talentModel.isSpecialRule(this.version)
                ? 0
                : (t.multiple ?? 1)),
          );
  ...
  get numberOfRefits() {
    return (
      refitCalculator(this) + (this.hasTalent('Experimental Vessel') ? 2 : 0)
    );
  }
  ...
  getBaseSystem(system: System) {
    ...
    if (this.spaceframeModel) {
      if (this.stereotype === Stereotype.SoloStarship) {
        ...
        if (
          this.spaceframeModel.isMissionPodAvailable &&
          this.missionPodModel
        ) {
  ...
  getQualifierForTalent(talentName: string) {
    if (
      talentName === TALENT_NAME_MISSION_POD &&
      this.missionPodModel != null
    ) {
  ...
  getMissionPodOverlapTalents() {
    ...
    if (this.spaceframeModel && this.stereotype !== Stereotype.SoloStarship) {
      this.spaceframeModel
        .talentsEffectiveForDate(this.serviceYear)
        .forEach((t) => grantedTalentNames.add(t.name));
    }
    if (
  ...
  getValidMissionPodReplacementTalents() {
    ...
    const grantedTalentNames = new Set<string>();
    this.talentsWithoutAdditional.forEach((t) =>
      grantedTalentNames.add(t.name),
    );
  ...
  hasUnreplacedMissionPodOverlaps() {
    return this.getMissionPodOverlapTalents().some((t) => {
      const index =
        this.missionPodModel?.talents.findIndex((p) => p.name === t.name) ?? -1;
  ...
  get talentsWithoutAdditional() {
    ...
    if (this.serviceRecordStep?.removedTalent != null) {
      ...
      result = result.filter(
        (t) => t.name !== this.serviceRecordStep?.removedTalent,
      );
    }
    ...
    if (this.missionPodModel && this.stereotype !== Stereotype.SoloStarship) {
      const overlappingTalentNames = this.getMissionPodOverlapTalents().map(
        (t) => t.name,
      );
  ...
  get talents(): SelectedTalent[] {
    ...
    this.advancementSteps
      .filter(
        (s) => s.choice === StarshipAdvancementChoice.Talent && s.value != null,
      )
      .forEach((s) => {
  ...
  get talentsWithoutSpecialRules(): SelectedTalent[] {
    return (
      this.talents?.filter((t) => !t.talentModel.isSpecialRule(this.version)) ??
      []
    );
  }
  ...
  private getNonSpaceframeTalentSelectionList() {
    ...
    if (
      ...
      this.addTalent(
        new SelectedTalent(this.missionProfileStep?.talent?.talent),
        talents,
      );
    }
  ...
  get shields() {
    if (this.departments) {
      let base =
        this.getSystemValue(System.Structure) +
        this.departments[Department.Security];
      if (this.version > 1) {
        base += this.scale;
      }
      if (this.buildType !== ShipBuildType.Starship) {
    ...
    if (spaceframe) {
      for (var attack of spaceframe.attacks) {
        for (let weapon of this.version === 1
          ...
          if (
            weapon.name === 'Spatial Torpedoes' &&
            this.hasTalent('Nuclear Warheads')
          ) {
            // skip it
            ...
            result.push(weapon);
          } else if (attack.indexOf(weapon.name) >= 0) {
            // Tractor or Grappler
  ...
  get systems() {
    ...
    this.advancementSteps
      .filter(
        (s) => s.choice === StarshipAdvancementChoice.System && s.value != null,
      )
      .forEach((s) => {
  ...
  get departments() {
    ...
    if (this.spaceframeModel !== undefined) {
      ...
      DepartmentsHelper.instance
        .getDepartments()
        .forEach((d) => (result[d] = this.simpleStats.departments[d]));
    }

    this.advancementSteps
      .filter(
        (s) =>
          s.choice === StarshipAdvancementChoice.Department && s.value != null,
      )
      .forEach((s) => {
  ...
  getDiceForWeapon(weapon: Weapon) {
    if (weapon.isTractorOrGrappler) {
      ...
      if (
        weapon.type === WeaponType.ENERGY &&
        this.hasTalent('Independent Phaser Supply')
      ) {
        dice += 1;
      }
  ...
  get totalAvailableSystemPoints(): number {
    return BuildPoints.systemPointsForType(
      this.buildType,
      this.spaceframeModel.serviceYear,
      this.type,
      this.scale,
      this.version,
    );
  ...
  public copy(): Starship {
    ...
    result.missionPodReplacements = this.missionPodReplacements.map((r) =>
      r?.copy(),
    );
