...
export class SelectedTalent implements ITalent {
  ...
  get isCustom() {
    return this.talent === TALENT_NAME_CUSTOM_TALENT;
  }

  get displayName() {
    if (this.isCustom && this.customTalentName?.length) {
      return this.customTalentName;
    } else {
      ...
      if (
        this.talent === TALENT_NAME_AUGMENTED_ABILITY &&
        this.attribute != null
      ) {
        name +=
          ' (' +
          i18next.t(
            makeKey('Construct.attribute.', Attribute[this.attribute]),
          ) +
          ')';
      }
      ...
      if (
        [
          ...
          TALENT_NAME_IM_A_DOCTOR_NOT_A,
          TALENT_NAME_CAUTIOUS,
        ].includes(this.talent) &&
        this.department != null
      ) {
        name +=
          ' (' +
          i18next.t(
            makeKey('Construct.discipline.', Department[this.department]),
          ) +
          ')';
      }
      ...
      if (
        ...
        name +=
          ' (' +
          i18next.t(
            makeKey('Construct.department.', Department[this.department]),
          ) +
          ')';
      }
      ...
      if (
        ...
        const weaponName =
          this.weapon instanceof Weapon
            ? (this.weapon as Weapon).name
            : (this.weapon as string);
        name +=
          ' (' + i18next.t('Construct.other.weapon') + ':' + weaponName + ')';
      }

      if (
        TALENT_NAME_ADDITIONAL_PROPULSION_SYSTEM === this.talent &&
        this.selection != null
      ) {
        name +=
          ' (' +
          (PropulsionSystemModel.getByType(
            this.selection as PropulsionSystemType,
          )?.localizedName ?? '') +
          ')';
      }
      ...
      if (
        ...
        const choice =
          this.selection === AttackType.Melee
            ? i18next.t('Weapon.common.melee')
            : i18next.t('Weapon.common.ranged');
        name += ' (' + choice + ')';
      }

      if (
        this.talent === TALENT_NAME_REDUNDANT_SYSTEMS &&
        this.selection != null
      ) {
        name += ' (' + this.selection + ')';
      } else if (
        ...
        name +=
          ' (' +
          i18next.t(makeKey('Construct.system.', System[this.system])) +
          ')';
      }
