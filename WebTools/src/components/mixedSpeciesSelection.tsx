...
const MixedSpeciesSelection: React.FC<ICharacterProperties> = ({
  ...
  const [primarySpecies, setPrimarySpecies] = useState(
    character?.speciesStep?.species === Species.Custom
      ? undefined
      : (character?.speciesStep?.species as Species),
  );
  const [secondarySpecies, setSecondarySpecies] = useState(
    character?.speciesStep?.mixedSpecies,
  );
  ...
  const assignSecondarySepecies = (species: Species) => {
    ...
    if (primarySpecies != null) {
      ...
      store.dispatch(
        setCharacterSpecies(
          primarySpecies,
          character?.speciesStep?.attributes ?? attributes,
          species,
          undefined,
          undefined,
  ...
  const primarySpeciesRows = SpeciesHelper.getPrimarySpecies(
    character.type,
    true,
    character,
  ).map((s, i) => {
    ...
    const rowBackground =
      Window.isCompact() && primarySpecies === s.id ? '#9179B7' : '';
  ...
  const secondarySpeciesRows = SpeciesHelper.getSpecies(character.type)
    .filter(
      (s) =>
        s.id !== Species.LiberatedBorg &&
        s.id !== Species.Borg &&
        s.id !== Species.Kobali &&
        s.id !== Species.CyberneticallyEnhanced,
    )
    .map((s, i) => {
      ...
      const rowBackground =
        Window.isCompact() && secondarySpecies === s.id ? '#9179B7' : '';
      ...
      <div className="row">
        <div className="col-lg-6 mt-3">
          ...
          <table className="selection-list">
            ...
            <tbody>{primarySpeciesRows}</tbody>
          </table>
        </div>
        <div className="col-lg-6 mt-3">
          ...
          <table className="selection-list">
            ...
            <tbody>{secondarySpeciesRows}</tbody>
          </table>
        </div>
      </div>
