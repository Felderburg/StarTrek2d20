...
import type { ICharacterSheet } from '../exportpdf/icharactersheet';
import { SheetTag } from '../exportpdf/icharactersheet';
...
> = ({ sheets, suffix, construct }) => {
  ...
  const exportPdf = async () => {
    ...
    if (sheet) {
      const existingPdfBytes = await fetch(
        sheet.getPdfUrl(construct.type),
      ...
      download(
        pdfBytes,
        sheet.createFileName(suffix, construct),
        'application/pdf',
      );
    }
  ...
  const filteredSheets = (selectedTags: SheetTag[]) => {
    return sheets.filter((s) => {
      ...
      selectedTags.forEach(
        (t) => (result = result && s.getTags().indexOf(t) >= 0),
      );
      return result;
    });
  };
  ...
  allTags.sort((t1, t2) =>
    t(makeKey('SheetTag.', SheetTag[t1])).localeCompare(
  ...
  const sheetList = filtered.map((s, i) => {
    const selected =
      s.getName() === selection.getName()
        ? 'sheet-selection-item selected'
        : 'sheet-selection-item';
    const overlay =
      s.getName() === selection.getName() ? (
        <img className="overlay" src="/static/img/check.png" alt="checkmark" />
      ) : undefined;
    ...
    return (
      <div
        ...
        key={'character-sheet-' + i}
      >
        <div className="sheet-selection-item-name">{s.getName()}</div>
        <div className="sheet-selection-item-thumbnail">
          <img
            className="thumbnail"
            src={s.getThumbnailUrl()}
            alt={i18n.t('CharacterSheetDialog.thumbnail')}
            width={200}
          />
          {overlay}
        </div>
  ...
  return (
    <div>
      <div className="mb-3">
        {allTags.map((type) => (
          <SelectablePill
            name={t(makeKey('SheetTag.', SheetTag[type]))}
            onClick={(active) => toggleTag(type, active)}
            key={'tag-' + SheetTag[type]}
          />
        ))}
      </div>
      <div className="style-selections">{sheetList}</div>
      <div className="text-center">
        ...
        >
          {i18n.t('Common.button.exportPdf')}
        </Button>
      </div>
    </div>
  );
