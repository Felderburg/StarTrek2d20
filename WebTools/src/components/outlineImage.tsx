...
const OutlineImage: React.FC<IOutlineImageProperties> = ({starship, size}) => {
    ...
    if (starship) {
        ...
        return svg ? (<div className='d-none d-md-block text-center mb-3'><img src={'data:image/svg+xml;base64,' + Buffer.from(svg, 'utf8').toString('base64')}
            alt={starship instanceof Starship ? starship.className : "Station"}
            width={width} height={height} style={{maxWidth: "100%"}}/></div>) : null;
    } else {
        return null;
    }
}
