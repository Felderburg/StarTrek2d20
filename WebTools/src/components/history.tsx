...
class History extends React.Component<IHistoryProperties, {}> {
  ...
  render() {
    return (
      <>
        <div
          className="sheet-bg"
          style={{ display: this.props.showHistory ? 'block' : 'none' }}
      ...
      return (
        <>
          {character?.speciesStep ? (
            this.renderPageTitleLink(PageIdentity.Species)
          ) : (
            <div>No history</div>
          )}
          {character?.environmentStep
            ? this.renderPageTitleLink(PageIdentity.Environment)
            : undefined}
          {character?.upbringingStep
            ? this.renderPageTitleLink(PageIdentity.Upbringing)
            : undefined}
          {character?.educationStep
            ? character.type === CharacterType.Child
              ? this.renderPageTitleLink(PageIdentity.ChildEducationPage)
              : this.renderPageTitleLink(PageIdentity.Career)
            : undefined}
          {character?.careerStep != null
            ? this.renderPageTitleLink(PageIdentity.CareerLength)
            : undefined}
          {character?.hasCareerEvents
            ? this.renderPageTitleLink(PageIdentity.CareerEvent1)
            : undefined}
          {character?.careerEvents?.length > 1
            ? this.renderPageTitleLink(PageIdentity.CareerEvent2)
            : undefined}
          {character?.finishingStep
            ? this.renderPageTitleLink(PageIdentity.AttributesAndDisciplines)
            : undefined}
        </>
      );
    }
  ...
  renderSoloCharacterHistory() {
    ...
    return (
      <>
        {character?.speciesStep
          ? this.renderPageTitleLink(PageIdentity.SoloSpecies)
          : undefined}
        {character?.environmentStep
          ? this.renderPageTitleLink(PageIdentity.Environment)
          : undefined}
        {character?.upbringingStep
          ? this.renderPageTitleLink(PageIdentity.SoloEarlyOutlook)
          : undefined}
        {character?.educationStep
          ? this.renderPageTitleLink(PageIdentity.SoloEducationType)
          : undefined}
        {character?.careerStep != null
          ? this.renderPageTitleLink(PageIdentity.CareerLength)
          : undefined}
        {character?.hasCareerEvents
          ? this.renderPageTitleLink(PageIdentity.SoloCareerEvent1)
          : undefined}
        {character?.careerEvents?.length > 1
          ? this.renderPageTitleLink(PageIdentity.SoloCareerEvent2)
          : undefined}
        {character?.finishingStep
          ? this.renderPageTitleLink(PageIdentity.SoloFinishingTouches)
          : undefined}
      </>
    );
  }
    ...
    return (
      <>
        <div
          className="history-item"
          key="pre-0"
          onClick={() => this.goToPage(PageIdentity.Era)}
        >
          Era
        </div>
        <div
          className="history-item"
          key="pre-1"
          onClick={() => this.goToPage(PageIdentity.ToolSelection)}
        >
          Registry
        </div>
