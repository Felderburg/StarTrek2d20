...
export const ExportToPdfButton: React.FC<IExportToPdfButtonProperties> = ({
  className,
  construct,
}) => {
        ...
        ({ CharacterSheetRegistry }) => {
          ...
          if (construct instanceof Station) {
            CharacterSheetDialog.show(
              CharacterSheetRegistry.getStationSheets(construct),
              'station',
              construct,
            );
          } else if (construct instanceof Starship) {
            CharacterSheetDialog.show(
              CharacterSheetRegistry.getStarshipSheets(construct),
              'starship',
              construct,
            );
          } else if (construct instanceof Creature) {
            CharacterSheetDialog.show(
              [CharacterSheetRegistry.getCreatureSheet()],
              'creature',
              construct,
            );
          }
