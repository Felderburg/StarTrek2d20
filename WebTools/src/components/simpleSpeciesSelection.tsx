...
export class SimpleSpeciesSelection extends React.Component<
  ISimpleSpeciesSelectionProperties,
  {}
> {
  ...
  render() {
    var species = this.props.species.map((s, i) => {
      ...
      const talents =
        s.id === Species.Changeling ? (
          <div>Morphogenic Matrix</div>
        ) : (
          s.talents.map((t, i) => {
            return t.isAvailableExcludingSpecies(this.props.character) ? (
      ...
      return (
        <tr
          key={i}
          onClick={() => {
            if (Window.isCompact()) this.props.onSelection(s);
          }}
        >
