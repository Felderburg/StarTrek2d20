...
const SpeciesSelection: React.FC<ISpeciesSelectionProperties> = ({
  ...
  const speciesOptions = SpeciesHelper.getPrimarySpecies(
    allowAllSpecies ? CharacterType.Starfleet : character.type,
    false,
    character,
  );
  ...
  const species = visibleSpeciesOptions
    .filter(
      (s) =>
        ...
        s.name.toLocaleLowerCase().indexOf(searchTerm.toLocaleLowerCase()) >=
          0 ||
        s.localizedName
          .toLocaleLowerCase()
    ...
    .map((s, i) => {
      ...
      const talents = s.talents.map((t, i) => {
        return t.isAvailableExcludingSpecies(character) ? (
          <div key={i}>{t.localizedDisplayName}</div>
        ) : (
          <span key={i}></span>
        );
      });
      ...
      if (character.version > 1) {
        ...
        if (
          ...
          speciesAbility = (
            <div className="mb-2">
              {ability.name + ' (' + t('Construct.other.speciesAbility') + ')'}
            </div>
          );
        }
      }
      const sources = s.sources.map((s) => (
        <div key={'source-' + s}>{SourcesHelper.getSourceName([s])}</div>
      ));
      ...
      return (
        <tr
          key={i}
          onClick={() => {
            if (Window.isCompact()) onSelection(s.id);
          }}
        >
  ...
  const buttonTitle = (
    <>
      <img
        ...
        className="me-1"
        alt={t('SpeciesPage.rollCoreSpecies')}
      />{' '}
      {buttonLabel}
    </>
  );
        ...
        <div className="col-md-6 my-4">
          {Character.isSpeciesListLimited(character) && !allowAllSpecies ? (
            <Button
              ...
              onClick={() =>
                setRandomSpecies(determineRandomSpecies(RandomMode.All))
              }
            >
              <>
                <img
            ...
            >
              ...
              {hasSource(Source.AlphaQuadrant) ? (
                <Dropdown.Item eventKey={RandomMode.AlphaQuadrant}>
                  Alpha Quadrant
                </Dropdown.Item>
              ) : undefined}
              {hasSource(Source.BetaQuadrant) ? (
                <Dropdown.Item eventKey={RandomMode.BetaQuadrant}>
                  Beta Quadrant
                </Dropdown.Item>
              ) : undefined}
              {hasSource(Source.GammaQuadrant) ? (
                <Dropdown.Item eventKey={RandomMode.GammaQuadrant}>
                  Gamma Quadrant
                </Dropdown.Item>
              ) : undefined}
              {hasSource(Source.DeltaQuadrant) ? (
                <Dropdown.Item eventKey={RandomMode.DeltaQuadrant}>
                  Delta Quadrant
                </Dropdown.Item>
              ) : undefined}
            </SplitButton>
        ...
        <thead>
          <tr>
            ...
            <td className="d-none d-md-table-cell">
              <b>{t('Construct.other.sources')}</b>
            </td>
            <td>
              <b>{t('Construct.other.talentOptions')}</b>
            </td>
