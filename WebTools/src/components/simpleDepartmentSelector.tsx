...
export const SimpleDepartmentSelector: React.FC<
  ISimpleDepartmentSelectorProperties
> = ({ onSelectDepartment, isChecked, character, isUpdateable }) => {
  ...
  const isIncrementable = (s: Department, departments: number[]) => {
    if (departments[s] === Character.maxDepartment(character)) {
      ...
      character.hasMaxedDepartment() &&
      departments[s] === Character.ABSOLUTE_MAX_DEPARTMENT - 1
    ) {
      return false;
    } else {
      return true;
  ...
  if (isUpdateable == null) {
    isUpdateable = (d, character) => {
      return isIncrementable(d, character.departments);
    };
  }
...
export const StarshipDepartmentSelector: React.FC<
  IStarshipDepartmentSelectorProperties
> = ({ onSelectDepartment, isChecked, starship, isUpdateable }) => {
  const { t } = useTranslation();

  const departments = starship.departments;
  if (isUpdateable == null) {
    isUpdateable = (d, starship) => {
      return true;
    };
  }
