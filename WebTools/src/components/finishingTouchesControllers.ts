...
export class FinishingTouchesAttributeController implements IAttributeController {
  ...
  canIncrease(attribute: Attribute): boolean {
    return (
      ...
      this.character.finishingStep?.attributes.filter((a) => a === attribute)
        .length <
        this.count - 1
    );
  }
  ...
  onIncrease(attribute: Attribute): void {
    store.dispatch(
      modifyCharacterAttribute(attribute, StepContext.FinishingTouches, true),
    );
  }
  onDecrease(attribute: Attribute): void {
    store.dispatch(
      modifyCharacterAttribute(attribute, StepContext.FinishingTouches, false),
    );
  }
  get instructions() {
    return [];
...
export class FinishingTouchesDisciplineController implements IDisciplineController {
  ...
  canIncrease(discipline: Department): boolean {
    return (
      ...
      this.character.finishingStep?.disciplines.filter((d) => d === discipline)
        .length <
        this.count - 1
    );
  }
  ...
  onIncrease(discipline: Department): void {
    store.dispatch(
      modifyCharacterDiscipline(discipline, StepContext.FinishingTouches, true),
    );
  }
