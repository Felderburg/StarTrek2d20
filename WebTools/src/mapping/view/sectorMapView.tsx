      ...
      <div className="sectorMap">
        <svg viewBox="0 0 450 450" xmlns="<http://www.w3.org/2000/svg>">
          <path
            ...
            fill="none"
            stroke={purple}
            d="M105,25 L105,425 M185,25 L185,425 M265,25 L265,425 M345,25 L345,425"
          />
          <path
            strokeWidth="1"
            strokeOpacity="50%"
            fill="none"
            stroke={purple}
            d="M25,105 L425,105 M25,185 L425,185 M25,265 L425,265 M25,345 L425,345 "
          />
          <path
            strokeWidth="1.5"
            stroke={purple}
            fill="none"
            d="M25,25 L25,425 L425,425 L425,25 Z"
          />
  ...
  renderStarSystems() {
    ...
    return systems.map((s, i) => {
      ...
      if (s.isBinary) {
        ...
        let r2 = Math.max(
          1,
          Math.sqrt(s.companionStar.spectralClass.radius.midpoint * 50),
        );
        ...
        return (
          ...
          >
            <circle
              cx={this.mapCoordinateSpaceX(s.sectorCoordinates.x) - r1 / 2}
              cy={this.mapCoordinateSpaceY(s.sectorCoordinates.y) - r1 / 2}
              stroke="black"
              strokeWidth={0.25}
              fill={colour.asHex()}
              r={r1}
            />
            <circle
              cx={this.mapCoordinateSpaceX(s.sectorCoordinates.x) + r2 / 2}
              cy={this.mapCoordinateSpaceY(s.sectorCoordinates.y) + r2 / 2}
              stroke="black"
              strokeWidth={0.25}
              fill={colour2.asHex()}
              r={r2}
            />
            <circle
              cx={this.mapCoordinateSpaceX(s.sectorCoordinates.x)}
              cy={this.mapCoordinateSpaceY(s.sectorCoordinates.y)}
              strokeWidth="1"
              stroke={colour.asHex()}
              fill="none"
              r={r + 5}
