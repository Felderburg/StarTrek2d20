...
export class StarView extends React.Component<IStarViewProperties, {}> {
  render() {
    return this.props.star ? (
      <div className="col mb-4 pe-3">
        ...
        <DataValueRow name="Spectral Class:">
          {this.props.star ? this.props.star.description : ''}
        </DataValueRow>
        {this.renderCompanionType()}
        {this.renderCompanionRadius()}
        <DataValueRow name="Mass:">
          {this.props.star ? this.props.star.mass.toFixed(2) + ' Sols' : ''}
        </DataValueRow>
        <DataValueRow name="Mass:">
          <span>
            {this.renderMassInKg()}
  ...
  renderCompanionType() {
    if (this.props.companionType == null) {
      ...
      return (
        <DataValueRow name="Companion Type:">
          {this.props.companionType === CompanionType.Close
            ? 'Close'
            : 'Distant'}
        </DataValueRow>
      );
  ...
  renderCompanionRadius() {
    if (this.props.orbitalRadius == null) {
      ...
      return (
        <DataValueRow name="Orbital Radius:">
          {this.props.orbitalRadius.toFixed(2) + ' AUs'}
        </DataValueRow>
      );
    }
  }
  ...
  renderLuminosity() {
    if (this.props.star.luminosityValue != null) {
      return (
        <DataValueRow name="Luminosity:">
          {(this.props.star.luminosityValue > 100
            ? this.props.star.luminosityValue.toFixed(0)
            : this.props.star.luminosityValue.toFixed(4)) + ' Sols'}
        </DataValueRow>
      );
    } else {
      return undefined;
