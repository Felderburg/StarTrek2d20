...
export class EditableHeader extends React.Component<
  IEditableHeaderProperties,
  IEditableHeaderState
> {
  ...
  render() {
    ...
    return (
      <h1 className="header-text visible-on-hover">
        <div>
          <div className="d-flex align-items-center">
            {this.props.prefix ? (
              <>
                <div>{this.props.prefix + separator}</div>{' '}
    ...
    if (this.state.editMode) {
      return (
        <input
          ...
          onKeyPress={(e) => {
            if (e.charCode === 13) this.toggleEditMode();
          }}
