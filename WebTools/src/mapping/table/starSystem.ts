import type { NotableSpatialPhenomenonModel } from './notableSpacialPhenomena';
import type { SectorCoordinates } from './sector';
import type { Star } from './star';
import type { World } from './world';
import { GasGiantDetails } from './world';
...
export class StarSystem {
  ...
  get luminosityValue() {
    if (
      ...
      this.companionType === CompanionType.Close
    ) {
  ...
  isInGardenZone(radius: number) {
    return (
      this.gardenZoneInnerRadius >= radius &&
      this.gardenZoneOuterRadius < radius
    );
  }
