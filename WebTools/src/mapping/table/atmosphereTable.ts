...
export class AtmosphereDetails {
  ...
  get attributeList(): WorldAttribute[] {
    ...
    if (this.density != null) {
      result.push(
        new WorldAttribute(
          i18next.t('World.attribute.atmosphericDensity'),
          i18next.t(
            makeKey(
              'World.atmosphericDensity.',
              AtmosphericDensityType[this.density],
            ),
          ),
        ),
      );
    ...
    if (this.contaiminants?.length) {
      result.push(
        new WorldAttribute(
          i18next.t('World.attribute.atmosphericContaminants'),
          this.contaiminants
            .map((c) =>
              i18next.t(
                makeKey(
                  'World.atmosphericContaminant.',
                  AtmosphericContaminantType[c],
                ),
              ),
            )
            .join(', '),
...
const atmosphereTableClassE = () => {
  return new AtmosphereDetails(
    AtmosphereType.HydrogenCompoundsAndReactiveGases,
  );
};
    ...
    case 20:
      ...
      more.forEach((c) => {
        if (result.indexOf(c) < 0) {
          if (
            c === AtmosphericContaminantType.LowOxygen &&
            result.indexOf(AtmosphericContaminantType.HighOxygen) >= 0
          ) {
            // skip it
          } else if (
            c === AtmosphericContaminantType.HighOxygen &&
            result.indexOf(AtmosphericContaminantType.LowOxygen) >= 0
          ) {
            // skip it
          } else {
            result.push(c);
