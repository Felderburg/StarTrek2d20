...
class LuminosityTableImpl {
  private values: LuminosityCrossReference[] = [
    ...
    new LuminosityCrossReference(SpectralClass.G, 0, LuminosityClass.VI, 0.32),
  ...
  public generateLuminosity(star: Star) {
    let options = this.values.filter(
      (l) =>
        l.luminosityClass === star.luminosityClass.id &&
    ...
    if (exactMatch != null) {
      ...
      if (o9 == null) {
        ...
        o9 = new LuminosityCrossReference(
          star.spectralClass.id,
          9,
          star.luminosityClass.id,
          o9Value,
        );
        options = [...options];
        options.push(o9);
  ...
  interpolateSelection(options: LuminosityCrossReference[], subClass: number) {
    ...
    return addNoiseToValue(
      lower.luminosity + (upper.luminosity - lower.luminosity) * ratio,
    );
  }
