...
const EditTablePageBase: React.FC<IEditTablePageProperties> = ({
  ...
  const [tableCollection, setTableCollection] = useState(() =>
    EditableTableCollection.from(initialTableCollection),
  );
  ...
  const saveTableCollection = () => {
    if (tableCollection.category == null) {
      ...
      if (initialTableCollection == null) {
        ...
        store.dispatch(
          replaceTableCollection(
            initialTableCollection.uuid,
            tableCollection.asTableCollection(),
          ),
        );
      }
    ...
    rows.forEach(
      (r) =>
        (error =
          ...
          isMissingName(r) ||
          isEndOverlapping(r) ||
          isStartOverlapping(r)),
    );
    return error;
  };

  return (
    <LcarsFrame activePage={PageIdentity.ViewTable}>
      <div id="app">
        <div className="page container ms-0">
            ...
            <ol className="breadcrumb">
              ...
              <li className="breadcrumb-item">
                <a
                  href="/index.html"
                  onClick={(e) =>
                    preventDefaultAnchorEvent(e, () => navigate('/tools'))
                  }
                >
                  {t('Page.title.otherTools')}
                </a>
              </li>
              <li className="breadcrumb-item">
                <a
                  href="/tools/table"
                  onClick={(e) =>
                    preventDefaultAnchorEvent(e, () => navigate('/tools/table'))
                  }
                >
                  {t('Page.title.tableList')}
                </a>
              </li>
              <li className="breadcrumb-item active" aria-current="page">
                {t('Page.title.viewTable')}
              </li>
            </ol>
          </nav>

          <Header>
            {initialTableCollection == null
              ? t('EditTablePage.header.createTable')
              : t('EditTablePage.header.modifyTable')}
          </Header>
            ...
            <div className="col mt-3">
              ...
              <DropDownSelect
                ...
                defaultValue={
                  otherSelected
                    ? OTHER
                    : categories.indexOf(tableCollection.category)
                }
          ...
          <div className="mt-3">
            ...
            <table className="table table-dark mt-3">
              ...
              <thead>
                <tr>
                  <th className="bg-black">From</th>
                  <th className="bg-black">To</th>
                ...
                {tableCollection.mainTable.rows.map((r, i) => (
                  <tr key={'row-' + r.key}>
                    <td>
                      <InputField
                        value={r.from}
                        onChange={(value) => selectRowFrom(value, i)}
                        maxLength={2}
                        className="form-control bg-black rounded-1"
                        max={20}
                        min={1}
                    ...
                    <td>
                      <InputField
                        value={r.to}
                        onChange={(value) => selectRowTo(value, i)}
                        max={20}
                        min={1}
          ...
          <div className="text-end mt-4">
            <Button size="sm" onClick={() => saveTableCollection()}>
...
const mapStateToProps = (state) => {
  return {
    categories: Array.from(
      new Set<string>(
        state.table?.collections?.map((c) => c.category) ?? [],
      ).values(),
    ),
