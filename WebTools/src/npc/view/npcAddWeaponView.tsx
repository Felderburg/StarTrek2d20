...
export const NpcAddWeaponView: React.FC<IAddWeaponViewProperties> = ({character, onClose, addWeapon}) => {

    const { t } = useTranslation();
    const [ selectedType, setSelectedType ] = useState<PersonalWeaponType|undefined>(undefined);

    const existingTypes = character.determineWeapons()
    ...
    const items = PersonalWeapons.instance(character.version).allTypes()
        ...
        <tr key={"weapon-" + PersonalWeaponType[w.personalWeaponType]}>
            <td className="align-middle"><CheckBox isChecked={selectedType === w.personalWeaponType}
