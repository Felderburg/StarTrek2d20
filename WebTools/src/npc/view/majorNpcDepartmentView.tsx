...
const MajorNpcDepartmentView: React.FC<ICharacterProperties> = ({character}) => {

    const { t } = useTranslation();

    useEffect(() => {
        if (!(character.npcGenerationStep?.departments?.filter(d => d != null)?.length)) {
    ...
    const canIncreaseDepartment = (department: Department) => {
        ...
        if (character.hasMaxedDepartment()) {
            return total < NpcTypes.departmentPointCount(NpcType.Major) && departments[department] < (Character.ABSOLUTE_MAX_DEPARTMENT - 1);
        } else {
            return total < NpcTypes.departmentPointCount(NpcType.Major) && departments[department] < Character.ABSOLUTE_MAX_DEPARTMENT;
        }
    }
    ...
    return (<>
        <div className="stats-row mt-4">
            <StatControl statName={t(makeKey('Construct.discipline.', Department[Department.Command]))} value={getDepartment(Department.Command)}
            ...
            <StatControl statName={t(makeKey('Construct.discipline.', Department[Department.Engineering]))} value={getDepartment(Department.Engineering)}
            ...
            <StatControl statName={t(makeKey('Construct.discipline.', Department[Department.Medicine]))} value={getDepartment(Department.Medicine)}
        ...
        <div className="stats-row">
            <StatControl statName={t(makeKey('Construct.discipline.', Department[Department.Conn]))} value={getDepartment(Department.Conn)}
            ...
            <StatControl statName={t(makeKey('Construct.discipline.', Department[Department.Security]))} value={getDepartment(Department.Security)}
            ...
            <StatControl statName={t(makeKey('Construct.discipline.', Department[Department.Science]))} value={getDepartment(Department.Science)}
