...
const MajorNpcAttributeViewBase: React.FC<ICharacterProperties> = ({
  ...
  useEffect(() => {
    if (
      !character.npcGenerationStep?.attributes?.filter((d) => d != null)?.length
    ) {
      store.dispatch(
        setNpcCharacterAttributes(
          NpcTypes.attributePoints(character.npcGenerationStep?.type, true).map(
            (v) => v - 7,
          ),
        ),
      );
  ...
  const canIncreaseAttribute = (attribute: Attribute) => {
    ...
    if (character.hasMaxedAttribute()) {
      return (
        total < NpcTypes.attributePointCount(NpcType.Major) &&
        attributes[attribute] < Character.ABSOLUTE_MAX_ATTRIBUTE - 1
      );
    } else {
      return (
        total < NpcTypes.attributePointCount(NpcType.Major) &&
        attributes[attribute] < Character.ABSOLUTE_MAX_ATTRIBUTE
      );
    }
  };
