...
export const NpcAddEquipmentView: React.FC<IAddEquipmentViewProperties> = ({character, onClose, addEquipment}) => {
    ...
    const availableTypes = EquipmentHelper.instance.items
        ...
        .filter(e => !(e.type === EquipmentType.Clothing && existingTypes.includes(EquipmentType.Uniform)));

    const items = availableTypes.map(e => (
        <tr key={"equipment-" + EquipmentType[e.type]}>
            <td className="align-middle"><CheckBox isChecked={selectedType === e.type}
    ...
    return (<>
            ...
            <div className="col-12 col-md-8 offset-md-2">
                <table className="table table-dark">
                    <tbody>
                        {items}
                        <tr>
                        <td className="align-bottom"><CheckBox isChecked={selectedType === EquipmentType.Other}
                            ...
                            <td className="align-middle"><InputFieldAndLabel
                                id="other"
        ...
        <div className="text-center mt-4">
            <Button onClick={() => addSelectedEquipment()}>{t('Common.button.add')}</Button>
        </div>
    </>);
}