...
import { CustomStationSpaceframeStep, StandardStationSpaceframeStep, Station } from '../../src/common/station';
...
describe('Station creation (#82)', () => {
    test('creates a station with a custom frame step at the minimum scale', () => {
        ...
        expect(station.stationFrameStep).toBeInstanceOf(CustomStationSpaceframeStep);
    ...
    test('allocates the full system and department point totals evenly', () => {
        ...
        expect(sum(station.departments)).toBe(station.totalAvailableDepartmentPoints);
        expect(station.systems).toEqual(PointAllocator.allocatePointsEvenly(station.totalAvailableSystemPoints));
        expect(station.departments).toEqual(PointAllocator.allocatePointsEvenly(station.totalAvailableDepartmentPoints));
    });
    ...
    test('copy() preserves the station data', () => {
        ...
        expect(copy.allTraits).toEqual(["Starfleet Station", "Home Away from Home"]);
    });
});
...
describe('Station point totals (#82)', () => {
    ...
    test('allows departments above 5 only for stations above scale 12', () => {
        ...
        large.stationFrameStep = new StandardStationSpaceframeStep(StationFrame.NarendraStationType);
...
describe('Standard station frames (#82)', () => {
    test('Spacedock is a scale 16 frame with full points and talents', () => {
        ...
        expect(sum(spacedock.systems)).toBe(Station.totalAvailableSystemPointsForScale(16));
        expect(sum(spacedock.departments)).toBe(Station.totalAvailableDepartmentPointsForScale(16));
    ...
    test('provides an International Space Station frame at scale 3', () => {
        const iss = StationFrameModel.getById(StationFrame.InternationalSpaceStation);
...
describe('Station traits and docking (#82)', () => {
    test('bases station traits on the character type', () => {
        ...
        const cardassian = Station.create(CharacterType.Cardassian, 2, Era.NextGeneration);
    ...
    test('computes docking ports and docking scale for large stations', () => {
        ...
        station.stationFrameStep = new StandardStationSpaceframeStep(StationFrame.Spacedock);
