...
import { isFlowStartPage, popPage, pushPage, workflowStepIndexForPage } from '../../src/common/pageHistory';
...
describe('pushPage', () => {
    test('appends the current page when navigating to a different page', () => {
        ...
        expect(pushPage(history, PageIdentity.Era, PageIdentity.ToolSelection)).toEqual([
            PageIdentity.Home,
    ...
    test('does not change history when navigating to the current page', () => {
        ...
        expect(pushPage(history, PageIdentity.Species, PageIdentity.Species)).toEqual([PageIdentity.Home]);
    });

    test('clears history when navigating to a flow start page', () => {
        const history = [PageIdentity.Home, PageIdentity.SourceSelection, PageIdentity.Era];
        expect(pushPage(history, PageIdentity.ToolSelection, PageIdentity.Home)).toEqual([]);
    });
...
describe('popPage', () => {
    ...
    test('returns the last page and the remaining history', () => {
        const history = [PageIdentity.Home, PageIdentity.SourceSelection, PageIdentity.Era];
        expect(popPage(history)).toEqual({ page: PageIdentity.Era, history: [PageIdentity.Home, PageIdentity.SourceSelection] });
    });
...
describe('workflowStepIndexForPage', () => {
    test('returns undefined when there is no workflow', () => {
        expect(workflowStepIndexForPage(undefined, PageIdentity.SpaceframeSelection)).toBeUndefined();
    });
    ...
    test('returns undefined when the page is not a workflow step', () => {
        ...
        expect(workflowStepIndexForPage(workflow, PageIdentity.Species)).toBeUndefined();
    });

    test('returns the matching workflow step index', () => {
        const workflow = ShipBuildWorkflow.createStarshipBuildWorkflow(2);
        expect(workflowStepIndexForPage(workflow, PageIdentity.SpaceframeSelection)).toBe(1);
        expect(workflowStepIndexForPage(workflow, PageIdentity.FinalStarshipDetails)).toBe(6);
    });
});
