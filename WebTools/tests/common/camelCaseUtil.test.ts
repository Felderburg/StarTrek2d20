...
describe('testing Camel Case Utility', () => {
  ...
  test('unusual characters', () => {
    expect(toCamelCase('Transporters & Replicators')).toBe(
      'transportersReplicators',
    );
  ...
  test('handle brackets', () => {
    expect(toCamelCase('Engineering/Science Affinity (Unofficial)')).toBe(
      'engineeringScienceAffinityUnofficial',
    );
  });
});
