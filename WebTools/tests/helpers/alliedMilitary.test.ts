...
} from '../../src/helpers/alliedMilitary';
...
describe('AllyHelper', () => {
  ...
  test('findOption finds by type', () => {
    const option = AllyHelper.instance.findOption(
  ...
  test('findOption returns undefined for unknown', () => {
    expect(
      AllyHelper.instance.findOption(999 as AlliedMilitaryType),
    ).toBeUndefined();
  });
  ...
  test('selectOptions excludes klingon when flag is false', () => {
    const options = AllyHelper.instance.selectOptions(
      Era.OriginalSeries,
      false,
    );
    options.forEach((o) =>
      expect(o.type).not.toBe(AlliedMilitaryType.KlingonDefenceForce),
    );
  });
});
