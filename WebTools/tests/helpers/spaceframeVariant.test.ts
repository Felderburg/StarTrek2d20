...
import {
  ...
  SpaceframeVariant,
} from '../../src/helpers/spaceframeVariant';
...
describe('SpaceframeVariantModel', () => {
  ...
  test('variantsBySpaceframe returns Excelsior variants', () => {
    const variants = SpaceframeVariantModel.variantsBySpaceframe(
      Spaceframe.Excelsior,
    );
    expect(variants.length).toBe(2);
    expect(variants.map((v) => v.id)).toContain(SpaceframeVariant.Excelsior);
    expect(variants.map((v) => v.id)).toContain(
  ...
  test('variantCodeByName returns undefined for unknown', () => {
    expect(
      SpaceframeVariantModel.variantCodeByName('Nonexistent'),
    ).toBeUndefined();
  });
});
