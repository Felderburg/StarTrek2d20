...
describe('EquipmentModel', () => {
  ...
  test('isArmour returns true for armour types', () => {
    expect(
      new EquipmentModel(EquipmentType.BodyArmour, 'x').isArmour,
    ).toBeTruthy();
    expect(
      new EquipmentModel(EquipmentType.ArmouredVest, 'x').isArmour,
    ).toBeTruthy();
    expect(
      new EquipmentModel(EquipmentType.EnvironmentSuit, 'x').isArmour,
    ).toBeTruthy();
    expect(
      new EquipmentModel(EquipmentType.PersonalForceField, 'x').isArmour,
    ).toBeTruthy();
  });

  test('isArmour returns false for non-armour types', () => {
    expect(
      new EquipmentModel(EquipmentType.Communicator, 'x').isArmour,
    ).toBeFalsy();
    expect(
      new EquipmentModel(EquipmentType.Tricorder, 'x').isArmour,
    ).toBeFalsy();
...
describe('EquipmentHelper', () => {
  ...
  test('findByType finds item', () => {
    const item = EquipmentHelper.instance.findByType(
  ...
  test('findByType returns undefined for unknown type', () => {
    expect(
      EquipmentHelper.instance.findByType(999 as EquipmentType),
    ).toBeUndefined();
  });
  ...
  test('findByTypeName returns undefined for unknown', () => {
    expect(
      EquipmentHelper.instance.findByTypeName('Nonexistent'),
    ).toBeUndefined();
  });
});
