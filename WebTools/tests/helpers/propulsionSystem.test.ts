...
} from '../../src/helpers/propulsionSystem';
...
describe('PropulsionSystemModel', () => {
  ...
  test('getByType finds system', () => {
    const ps = PropulsionSystemModel.getByType(
      PropulsionSystemType.QuantumSlipstreamDrive,
    );
  ...
  test('getByType returns undefined for unknown', () => {
    expect(
      PropulsionSystemModel.getByType(999 as PropulsionSystemType),
    ).toBeUndefined();
  });
